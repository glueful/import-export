...
final class ProcessImportBatchJob extends Job
{
    ...
    {
        try {
            $this->resolveRunner()->runImportBatch((string) ($this->getData()['batch_uuid'] ?? ''));
        } catch (\Throwable $e) {
            error_log('Import batch job failed without queue retry: ' . $e->getMessage());
        }
    }
